...
export function AiReceiptDrawer() {
  ...
  const renderConfidenceBadge = (confidence: number) => {
    ...
    if (confidence >= 0.7) {
      return (
        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 text-amber-700 rounded-md text-xs font-medium border border-amber-100">
  ...
  return (
    <Sheet open={store.isOpen} onOpenChange={(open) => !open && handleCancel()}>
      <SheetContent className="sm:max-w-[560px] w-full p-0 flex flex-col bg-[#FFFEFC] border-l-[#E8E7E5] overflow-hidden">
        {/* Header */}
        <div className="px-6 py-4 border-b border-[#E8E7E5] flex items-center justify-between shrink-0 bg-white">
          <SheetTitle className="flex items-center gap-2 text-lg text-[#37352F]">
          ...
          <div className={cn(
            ...
            store.statusBadge === "Draft" ? "bg-[#F7F6F3] text-[#9B9A97] border-[#E8E7E5]" :
              store.statusBadge === "Processing" ? "bg-amber-50 text-amber-600 border-amber-200" :
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {store.step !== "idle" && store.step !== "error" && (
            <div className="flex items-center gap-2 text-sm text-[#9B9A97] font-medium">
              <span className={cn(store.step === "uploading" ? (store.mode === "classification" ? "text-fuchsia-600" : "text-indigo-600") : "text-[#37352F]")}>① Upload</span>
              <span className="text-[#E8E7E5]">-</span>
              <span className={cn((store.step === "processing_ocr" || store.step === "processing_classification") ? (store.mode === "classification" ? "text-fuchsia-600 animate-pulse" : "text-indigo-600 animate-pulse") : store.step === "review" ? "text-[#37352F]" : "")}>② {store.mode === "classification" ? "Phân tích Ảnh" : "Trích xuất OCR"}</span>
              <span className="text-[#E8E7E5]">-</span>
          ...
          {store.step === "idle" && (
            <div className="flex flex-col items-center justify-center p-10 border-2 border-dashed border-[#E8E7E5] rounded-xl bg-[#F7F6F3] hover:bg-[#F0EFEA] transition-colors cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-4 shadow-sm border border-[#E8E7E5]">
                <ImageIcon className="w-6 h-6 text-[#9B9A97]" />
              </div>
              <p className="text-sm font-medium text-[#37352F] mb-1">Click để tải ảnh hóa đơn</p>
              <p className="text-xs text-[#9B9A97] text-center max-w-[200px]">Hỗ trợ JPG, PNG. Khuyên dùng ảnh rõ nét.</p>
          ...
          {store.imageUrl && (
            <div className="w-full rounded-xl overflow-hidden border border-[#E8E7E5] bg-[#F7F6F3] relative flex-shrink-0" style={{ maxHeight: store.step === "review" ? "200px" : "auto" }}>
              ...
              {(store.step === "uploading" || store.step === "processing_ocr" || store.step === "processing_classification") && (
                <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex flex-col items-center justify-center">
                  <div className="relative w-16 h-16 mb-4">
                    <div className="absolute inset-0 border-4 border-[#E8E7E5] rounded-full"></div>
                  ...
                  <p className="text-sm font-medium text-[#37352F]">AI đang phân tích hóa đơn...</p>
                </div>
              )}
            </div>
          )}
          ...
          {store.step === "error" && (
            <div className="flex flex-col items-center justify-center p-6 bg-red-50 border border-red-100 rounded-xl">
              ...
              <Button onClick={() => store.clearSession()} variant="outline" size="sm" className="bg-white">
                Thử ảnh khác
              </Button>
            </div>
          )}
            ...
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="merchantName" className="text-[#37352F]">{store.mode === "classification" ? "Tiêu đề (Tên giao dịch)" : "Tên cửa hàng"}</Label>
                  <Input
                    id="merchantName"
                    value={store.editableForm.merchantName}
                    onChange={(e) => store.updateEditableForm({ merchantName: e.target.value })}
                    className="bg-white border-[#E8E7E5]"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="totalAmount" className="text-[#37352F]">Số tiền (đ)</Label>
                  <Input
                    id="totalAmount"
                    type="number"
                    value={store.editableForm.totalAmount || ""}
                    onChange={(e) => store.updateEditableForm({ totalAmount: Number(e.target.value) })}
                    className="bg-white border-[#E8E7E5] font-semibold text-emerald-600"
                    placeholder="0"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="category" className="text-[#37352F]">Danh mục</Label>
                  <CategorySelect
                    type="EXPENSE"
                ...
                <div className="space-y-2">
                  <Label htmlFor="transactionDate" className="text-[#37352F]">Ngày giao dịch</Label>
                  <Input
                    id="transactionDate"
                    type="date"
        ...
        {/* Footer Actions */}
        <div className="px-6 py-4 border-t border-[#E8E7E5] bg-gray-50 flex items-center justify-end gap-3 shrink-0">
          <Button variant="ghost" onClick={handleCancel} className="text-[#5A5A57] hover:bg-[#E8E7E5]">
            {store.step === "review" ? "Lưu Nháp" : "Đóng"}
          </Button>

          {store.step === "review" && (
            <Button onClick={handleSubmit} className="bg-[#37352F] hover:bg-[#2F2D28] text-white">
              Xác nhận & Điền form
            </Button>
          )}
        </div>
