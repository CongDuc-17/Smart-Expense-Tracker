...
export function setupAiEventSubscribers() {
	eventBus.on(AppEvents.EXPENSE_IMAGE_UPLOADED, (payload: ExpenseImageUploadedEventPayload) => {
		console.log(`[AI Event] EXPENSE_IMAGE_UPLOADED for expense ${payload.expenseId}`);
		imageClassificationService.classify(payload.expenseId, payload.imageUrl).catch((err) => {
